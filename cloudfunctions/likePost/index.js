...
exports.main = async (event, context) => {
	...
	try {
		const record = await db.collection('likeList').where({
			_id: openid
		}).get()
		...
		const updateResult = await db.collection('likeList').doc(openid).update({
			data: {
				list: updated_data
			}
		});
		...
		try {
			const addResult = await db.collection('likeList').add({
				data: {
					_id: openid,
					list: [event.postId]
				}
