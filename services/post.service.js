...
async function createPost(newPostData) {
	...
	for (let fileId of newImgUrls) {
		let traceId = await imgSecCheck(newPostData._id, fileId);
...
async function editPost(newPostData) {
	...
	console.log("✏️post.service.js: editPost(): Editing Post: ", newPostData);

	// 第一步：获取原始版本的旧帖子
	const oldPostRes = await db.collection("posts").doc(newPostData._id).get();
	...
	if (filesToDelete.size > 0) {
		...
		console.log("✅ post.service.js: editPost(): Successfully deleted post images: ", Array.from(filesToDelete));
	}
	// 计算编辑后帖子里所有图片的链接 P = (C + N)
	// 先上传C里面的图片，并且把获取到的数据库里的新链接加到P里
	const newImgUrls = [...filesToKeep];
	const uploadResults = await uploadImages(newPostData._id, Array.from(filesToAdd));
	console.log("✅ post.service.js: editPost(): Successfully get all new images Urls: ", uploadResults);
	...
	// 第三步：把帖子的所有信息更新成新的
	const updateResult = await db.collection("posts").doc(newPostData._id).update({
		data: newPostData
	});
	console.log("✅ post.service.js: editPost(): Successfully updated the post: ", updateResult);
	return updateResult;
}
		...
		db.collection("posts").doc(postId).get({
			success: function(res) {
				...
				db.collection('posts').doc(postId).remove({
					success: res => {
						resolve(res);
						console.log("🚮 post.service.js: deletePost(): deleting post images: ", imageUrls);
